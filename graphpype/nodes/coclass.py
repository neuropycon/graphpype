...
class PrepareCoclassInputSpec(BaseInterfaceInputSpec):
    ...
    mod_files = traits.List(File(
        exists=True), desc='list of all files representing modularity assignement (in rada, lol files) for each subject', mandatory=True)

    node_corres_files = traits.List(File(
        exists=True), desc='list of all Pajek files (in txt format) to extract correspondance between nodes in rada analysis and original subject coordinates for each subject (as obtained from PrepRada)', mandatory=True)
...
class PrepareCoclassOutputSpec(TraitedSpec):
    ...
    group_coclass_matrix_file = File(
        exists=True, desc="all coclass matrices of the group in .npy (pickle format)")

    sum_coclass_matrix_file = File(
        exists=True, desc="sum of coclass matrix of the group in .npy (pickle format)")

    sum_possible_edge_matrix_file = File(
        exists=True, desc="sum of possible edges matrices of the group in .npy (pickle format)")

    norm_coclass_matrix_file = File(
        exists=True, desc="sum of coclass matrix normalized by possible edges matrix of the group in .npy (pickle format)")
    ...
    def _run_interface(self, runtime):
        ...
        if isdefined(self.inputs.gm_mask_coords_file) and isdefined(self.inputs.coords_files):
            ...
            group_coclass_matrix = np.zeros(
                (gm_mask_coords.shape[0], gm_mask_coords.shape[0], len(mod_files)), dtype=float)

            print(group_coclass_matrix.shape)

            if len(mod_files) != len(coords_files) or len(mod_files) != len(node_corres_files):
                print("warning, length of mod_files, coords_files and node_corres_files are imcompatible {} {} {}".format(
                    len(mod_files), len(coords_files), len(node_corres_files)))
            ...
            for index_file in range(len(mod_files)):
                ...
                if os.path.exists(mod_files[index_file]) and os.path.exists(node_corres_files[index_file]) and os.path.exists(coords_files[index_file]):
                    ...
                    print("Warning, one or more files between " +
                          mod_files[index_file] + ',' + node_corres_files[index_file] + ', ' + coords_files[index_file] + " do not exists")
            ...
            group_coclass_matrix = np.zeros(
                (gm_mask_labels.shape[0], gm_mask_labels.shape[0], len(mod_files)), dtype=float)
        ...
        norm_coclass_matrix = np.divide(np.array(sum_coclass_matrix, dtype=float), np.array(
            sum_possible_edge_matrix, dtype=float)) * 100
...
class DiffMatricesOutputSpec(TraitedSpec):
    ...
    diff_mat_file = File(
        exists=True, desc='Difference of Matrices (mat1 - mat2) in npy format', mandatory=True)

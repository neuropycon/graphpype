...
def test_neuropycon_data():
    ...
    assert os.path.exists(indexed_mask_file), ("warning, could not find path \
        {}, {}".format(indexed_mask_file, os.listdir(data_path)))
...
def test_return_data_img_from_roi_mask():
    ...
    assert all(data_img_vect_vals == np.unique(test_vect)), \
        ("Error, values in data_img_vect do not match values of test_vect")

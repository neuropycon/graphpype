...
def compute_rada_df(iter_path, df, radatools_version="3.2", mapflow=[], mapflow_name=""):
    ...
    if len(mapflow) == 0:
        ...
        if os.path.exists(path_length_file):
            ...
            mean_path_length, diameter, global_efficiency = get_path_length_from_info_dists_file(
                path_length_file)
        ...
        for i, cond in enumerate(mapflow):
            ...
            modularity_file = os.path.join(
                iter_path, "community_rada", "mapflow", "_community_rada"+str(i), "Z_List.lol")
            ...
            global_info_file = os.path.join(
                iter_path, net_prop_dir, "mapflow", "_" + net_prop_dir+str(i), "Z_List-info_global.txt")
            ...
            path_length_file = os.path.join(
                iter_path, net_prop_dir, "mapflow", "_" + net_prop_dir+str(i), "Z_List-info_dists.txt")
...
def compute_nodes_rada_df(local_dir, gm_coords, coords_file, labels_file, radatools_version="3.2"):
    ...
    if os.path.exists(coords_file) and os.path.exists(Pajek_file) and os.path.exists(labels_file):
        ...
        list_df.append(pd.DataFrame(np.concatenate((where_in_gm_mask, node_labels, node_coords), axis=1), columns=[
                       'Where_in_GM_mask', 'labels', 'MNI_x', 'MNI_y', 'MNI_z']))
    else:
    ...
    if os.path.exists(roles_file) and os.path.exists(part_coeff_file) and os.path.exists(Z_com_degree_file):
        ...
        list_df.append(pd.DataFrame(np.concatenate((node_roles, part_coeff, Z_com_degree), axis=1), columns=[
                       'Role_quality', 'Role_quantity', 'Participation_coefficient', 'Z_community_degree']))
...
def compute_signif_permuts(permut_df, permut_col="Seed", session_col="Session", start_col=0, stop_col=0, columns=[]):
    ...
    if not all(x in seed_index[1:] for x in expected_permut_indexes):
        print("Error, permut indexes should be consecutive and start with 0:  {} ".format(
            expected_permut_indexes))
    ...
    if session_col == -1 or len(permut_df[session_col].unique()) == 1:
        ...
        for index_col, col in enumerate(data_cols):
            ...
            sum_higher = np.sum(
                (permut_df[col].iloc[1:] > permut_df[col].iloc[0]).values.astype(int))
        ...
        if not all(val == 2 for val in list(count_elements.values())):
            print("Error, all permut indexes should have 2 and only 2 lines: {}".format(
                count_elements))
            # 0/0
            # return pd.DataFrame()
...
def compute_signif_node_prop(orig_df, list_permut_df, columns):
    ...
    for col in columns:
        ...
        assert col in orig_df.columns, "Error, {} not in orig columns {}".format(
            col, orig_df.columns)
        assert col in permut_df.columns, "Error, {} not in permut columns {}".format(
            col, permut_df.columns)
        ...
        frac_higher = np.array(sum_higher(
            orig_df[col], permut_df[col])+1, dtype=float)/float(len(permut_df.index) + 1)
...
def gather_diff_con_values(res_path, cond, nb_permuts, labels):
    ...
    if not os.path.exists(df_filename):
        ...
        pair_labels = [labels[i] + "_" + labels[j]
                       for i, j in zip(triu_indices_i.tolist(), triu_indices_j.tolist())]
        ...
        for seed in range(-1, nb_permuts):
            ...
            for sess in ['1', '2']:
                ...
                avg_cormat_file = os.path.join(
                    res_path, iter_dir, "prepare_mean_correl" + sess, "avg_cormat.npy")
